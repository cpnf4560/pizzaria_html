...
class LoginSystem {
        ...
        if (this.token) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log('✅ Login automático:', this.currentUser.name);
    ...
    async handleLogin(e) {
        ...
        if (!username || !password) {
            this.showLoginError('Username e password são obrigatórios');
            return;
        }
    ...
    async handleRegister(e) {
        ...
        try {
            ...
            console.log('🌐 URL da API:', `${this.API_BASE}/register`);
            ...
            const response = await fetch(`${this.API_BASE}/register`, {
                ...
                body: JSON.stringify({ username, password, name, morada, telefone, email })
            });
            ...
            if (response.ok && data.success) {
                console.log('✅ Registo bem-sucedido:', data.user.name);
    ...
    showWelcomePage() {
        ...
        if (nomeElement && this.currentUser) {
            nomeElement.textContent = `Olá, ${this.currentUser.name}!`;
        }
    ...
    async loadUserHistory() {
        ...
        try {
            const response = await fetch(`${this.API_BASE}/user/history`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${this.token}`,
                    'Content-Type': 'application/json'
        ...
        const historicoHTML = encomendas.map(encomenda => `
            <div class="historico-item">
                <div class="historico-header">
                    ...
                    <div class="historico-data">${new Date(encomenda.data_encomenda).toLocaleDateString('pt-PT', {
                        day: '2-digit',
                        month: 'long',
                        year: 'numeric',
                        hour: '2-digit',
                    ...
                    ${encomenda.pizzas && encomenda.pizzas.length > 0 ? 
                        encomenda.pizzas.map(pizza => `
                            <div class="historico-pizza">
                                ...
                                <span style="float: right; font-weight: bold; color: #00994d;">${pizza.quantidade}x €${pizza.preco}</span>
                            </div>
                        `).join('') 
